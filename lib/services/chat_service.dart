...
class ChatService {
  ...
  static Future<String?> createDirectChatRoom(String otherUserId) async {
    try {
      ...
      final otherUserData = UserProfile.fromMap(
        otherUser.data()!,
        otherUser.id,
      );
    ...
    return _firestore
        ...
        .map(
          (snapshot) => snapshot.docs
              ...
              ) // Filter in app instead of query
              .toList(),
        );
  }
  ...
  static Future<void> markMessagesAsReadUpTo(
    String chatRoomId,
    String messageId,
  ) async {
    try {
      ...
      if (foundTarget || messages.isNotEmpty) {
        ...
        await _firestore.collection('chatRooms').doc(chatRoomId).update({
          'lastReadBy.$currentUserId': messageId.isEmpty
              ? messages.last.id
              : messageId,
        });
      }
  ...
  static Future<List<UserProfile>> getParticipantProfiles(
    ...
    try {
      ...
      if (messages.docs.isNotEmpty) {
        ...
        if (latestMessage.isDeleted) {
          ...
          lastMessage =
              '${latestMessage.senderId}|${latestMessage.senderName}:DELETED_MESSAGE';
        } else {
  ...
  static Future<void> _sendSystemMessage({
    ...
    try {
      ...
      await _firestore.collection('chatRooms').doc(chatRoomId).update({
        'lastMessage': content,
        'lastMessageTime':
            FieldValue.serverTimestamp(), // Fixed: use lastMessageTime instead of lastMessageTimestamp
        'lastMessageSender':
            'system', // Fixed: use lastMessageSender instead of lastMessageSenderId
      });
    } catch (e) {
  ...
  static Future<List<UserProfile>> getAllUsersForGroupAdd([
  ...
  static Future<bool> hasUserReadUpToMessage(
    String chatRoomId,
    String userId,
    String messageId,
  ) async {
    try {
      final chatRoomDoc = await _firestore
          ...
          .get();
      final lastReadBy =
          chatRoomDoc.data()?['lastReadBy'] as Map<String, dynamic>?;
      ...
      final lastReadMessage = messages.docs.firstWhere(
        (doc) => doc.id == lastReadMessageId,
      );
      final targetMessage = messages.docs.firstWhere(
        (doc) => doc.id == messageId,
      );

      final lastReadTimestamp =
          lastReadMessage.data()['timestamp'] as Timestamp;
